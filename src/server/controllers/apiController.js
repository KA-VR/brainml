...
const createSynonym = (verb, relation) => {
  apoc.query('CREATE (n:Verb {name: "%verb%", usageRate: 1})', {verb}).exec()
    .then(response => {
      ...
      apoc.query('MATCH (n:Verb {name: "%verb%}), (m:Verb {name: "%relation%"}) CREATE (n)-[:SYN {confidence: 0}]->(m)', {verb, relation}).exec()
        .then(response2 => {
...
const getTags = (callback) => {
  apoc.query('MATCH (n)-[r:OUTPUT]->(m) return r.tags').exec()
    .then(response => {
      console.log(response);
      callback(response[0].data.reduce((memo, dataobj) => {
        return memo.concat(dataobj.row[0]);
      },[]));
    })
    .catch(err => {
...
const getFunction = (req, res) => {
  ...
  determineRoute(object, (route) => {
    ...
    apoc.query('MATCH (n:Verb {name:"%verb%"})-[:SYN*]->(m:Action)-[b:OUTPUT]->(r:Function) where any(x in b.tags where x = "%route%") return r', { verb, route }).exec()
      .then(response => {
        console.log(response);
